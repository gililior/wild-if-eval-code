...
from scripts.evaluate_llms.api_inference import NewData, InitGenerationsAPI
from scripts.evaluate_llms.constants import PROMPT_EVAL
import random
import json
...
def generate_parallel(obj, tasks):
    ...
    for task in tasks:
        ...
        for atomic in obj.data.get_constraints_for_task(task):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--eval_model", required=True,
                        help="judge model name as exposed on the inference endpoint")
    parser.add_argument("--to_eval", required=True,
                        help="path to the responses JSON to evaluate")
    ...
    parser.add_argument("--data", default="gililior/wild-if-eval",
